...
const CHANNEL_OBFUSCATION_CAPABILITY = 1 << 15;
...
function patchGatewayIdentifyPayload() {
    if (!GatewayConnectionStore?.getSocket) {
        log("gateway connection store was not available");
        return;
    }

    let socket: any;
    try {
        socket = GatewayConnectionStore.getSocket();
    } catch {}

    if (!socket || typeof socket.send !== "function") {
        log("gateway socket was not available");
        return;
    }

    safeRegisterPatch(() =>
        before("send", socket, (args) => {
            const [opcode, payload] = args;
            if (opcode !== IDENTIFY_OPCODE || !payload || typeof payload !== "object") return;

            // Patch the actual gateway payload, rather than only exported
            // experiment helpers. Discord's Metro modules retain local
            // references to those helpers, so replacing the export object is
            // not sufficient to change the IDENTIFY generated by GatewaySocket.
            const capabilities = Number(payload.capabilities);
            if (Number.isFinite(capabilities)) {
                payload.capabilities = capabilities & ~CHANNEL_OBFUSCATION_CAPABILITY;
            }
